...
for i in range(0, len(triplet_arrays)-1):
    ...
    for j in range(i+1, len(triplet_arrays)):
        ...
        if no_of_triplets > 0:
            div = no_of_triplets / no_of_current_triplet  # Divide the number of triplets
            protein_water_hbond.append(div)  # Store the divided values
            
        else:
            protein_water_hbond.append(np.nan)  # Append NaN if no common triplets
...
np.savetxt('intermittent_protein_water_hbond.dat', data_to_save, fmt="%d %.4f", delimiter='\t')
